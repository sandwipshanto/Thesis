...
\begin{enumerate}
    \item Develop and Validate Bangla-English Code-Mixed Attack Methodology
    
    \item Characterize Bangla-Specific Attack Patterns
    
    \item Evaluate Cross-Model Vulnerability Consistency
    
    \item Validate Tokenization Disruption as Attack Mechanism
\end{enumerate}
...
\begin{enumerate}
    \item First Bangla code-mixing jailbreaking study: Systematic evaluation of 230M speaker population previously untested in adversarial contexts (200 prompts across 10 categories, 3 major LLMs, 27,000 responses)
    
    \item Bangla-specific attack optimization: Discovery that perturbing English words within Banglish prompts is 85\% more effective than perturbing Bangla words
    
    \item Template ineffectiveness finding: Contrary to expectations, jailbreak templates reduce effectiveness for Bangla (46.2\% AASR with ``None'' template vs. 35.1-42.5\% with jailbreak templates)
    
    \item Tokenization mechanism application: Application of tokenization disruption hypothesis (empirically validated for Hindi-English via Integrated Gradients by Aswal \& Jaiswal, 2025) to Bangla-English context, with AASR patterns consistent with fragmentation-based explanation
    
    \item Romanization variability analysis: Identification of Bangla's non-standard romanization as a unique vulnerability creating multiple valid tokenization paths
    
    \item Scalable framework: Replicable methodology applicable to 20+ other Indic languages at $\sim$\$1.50-2.00 per language
\end{enumerate}
