...
\textbf{Limitation:}
\begin{itemize}
    \item Our study uses 200 prompts vs. 460 in the Hinglish study \citep{aswal2025}
    \item Dataset reduced by $\sim$57\% due to budget constraints ($\sim$\$2.50 available vs. $\sim$\$5-10 for full scale)
    \item Iterative scaling (50$\rightarrow$200 prompts) balanced validation with statistical power
    \item 75\% data collection (27,000/36,000 queries) due to experimental interruptions
\end{itemize}

\textbf{Impact:}
\begin{itemize}
    \item Results highly significant despite smaller scale (English$\rightarrow$CMP p=0.0070)
    \item Category-specific patterns well-represented (20 prompts per category)
    \item Statistical power sufficient for publication-quality findings (n=27,000)
\end{itemize}

\textbf{Mitigation:}
\begin{itemize}
    \item Balanced distribution across 10 categories (20 prompts each, 4$\times$ initial validation)
    \item Highly significant results achieved (p=0.0070 for main English$\rightarrow$CMP transition)
    \item Framework validated through iterative scaling, ready for 460-prompt expansion
    \item Resume functionality enables completion of remaining 9,000 queries if needed
\end{itemize}
