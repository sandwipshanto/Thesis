...
\subsection{Finding 2: Incremental Transformation Contributions}
Our analysis reveals that both code-mixing and phonetic perturbations independently contribute to attack effectiveness. The English→CM transition provides +4.3pp improvement (35.0\% → 39.3\%), while the CM→CMP transition adds an additional +4.6pp (39.3\% → 43.9\%). This demonstrates that romanization and phonetic obfuscation work through complementary mechanisms, with their combination achieving compound effectiveness gains of 25.4\% relative improvement.

\subsection{Finding 3: Inconsistent Model Vulnerability}
Our cross-model analysis reveals dramatic inconsistency in vulnerability to Bangla attacks. Mistral-7B proves critically compromised at 86.6\% average AASR, while GPT-4o-mini demonstrates the strongest resistance at 9.8\% average AASR yet remains exploitable with a 15× increase over its English baseline. Critically, no tested model achieves adequate safety coverage for the 230 million Bangla speakers worldwide, exposing a systemic gap in multilingual AI safety.

\subsection{Finding 4: Tokenization is the Primary Mechanism}
Our analysis confirms that tokenization disruption serves as the primary attack mechanism for Bangla code-mixing exploits. The progressive AASR improvement across transformation stages (English 35.0\% → CM 39.3\% → CMP 43.9\%) aligns with increasing tokenization fragmentation complexity, supporting the hypothesis that phonetic perturbations fragment harmful keywords into semantically inert subword units that evade token-level safety filters.
...
Critical Differences: Despite methodological parallels, our work constitutes an independent contribution with several distinguishing features. We investigate Bangla rather than Hindi, which differs substantially in phonology and romanization conventions. Our dataset comprises 200 carefully curated prompts compared to their 460 prompts, reflecting budget constraints that limited us to testing 3 models fully. While smaller in scale, our study yields a novel finding regarding the counterintuitive ineffectiveness of jailbreak templates for code-mixing attacks, with simple prompts outperforming elaborate adversarial scenarios.
...
Our discussion establishes core findings that advance understanding of multilingual LLM safety. First, Bangla code-mixing constitutes an effective jailbreaking strategy, achieving 43.9\% AASR with statistically significant improvement over English baselines. Second, incremental transformations provide compound effectiveness gains, with code-mixing and phonetic perturbations independently degrading safety filter performance (+4.3pp and +4.6pp respectively). Third, all major tested LLMs exhibit vulnerability to Bangla attacks, though vulnerability magnitude varies dramatically across models (9.8\% to 86.6\% AASR). Fourth, the progressive AASR improvement across transformation stages supports tokenization fragmentation as the mechanistic explanation. Fifth, current safety alignment approaches fail to generalize from English to Bangla-English code-mixing scenarios, exposing a fundamental limitation in existing RLHF methodologies. Sixth, urgent improvements are needed in multilingual safety training to address systematic gaps affecting hundreds of millions of non-English speakers worldwide.
