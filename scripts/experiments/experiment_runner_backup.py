...
class ExperimentRunner:
    ...
    def run_experiments(self, dry_run: bool = False):
        ...
        estimated_time_hours = total_queries / 500  # ~500 queries/hour
    ...
    def _run_single_configuration(
        ...
        # Get English prompts for judge evaluation
        english_prompts = prompt_df['hypothetical_scenario'].tolist()
        
        # Process in batches
        num_prompts = len(prompt_df)
        
        for batch_start in tqdm(range(0, num_prompts, batch_size), 
                                desc=f"  Querying {model_name}", 
                                leave=False):
            batch_end = min(batch_start + batch_size, num_prompts)
            batch_df = prompt_df.iloc[batch_start:batch_end]
